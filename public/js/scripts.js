    ...
    function updatePreview() {
      ...
      if (selectedColor) {
        ...
        if (previewContainer) {
          ...
          const bgColor = colorMap[selectedColor.value] || selectedColor.value.toLowerCase()
          previewContainer.style.backgroundColor = bgColor

          // Adjust text color for visibility
          if (["Black", "Navy Blue", "Maroon", "Forest Green"].includes(selectedColor.value)) {
        ...
        if (previewContainer) {
          ...
          switch (selectedPattern.value) {
            ...
            case "Striped":
              ...
              break
            case "Polka Dot":
              previewContainer.style.backgroundImage = "radial-gradient(circle, rgba(0,0,0,0.1) 2px, transparent 2px)"
              previewContainer.style.backgroundSize = "20px 20px"
              break
            case "Floral":
              // Simplified representation
              previewContainer.style.backgroundImage = 'url("/placeholder.svg?height=50&width=50")'
              previewContainer.style.backgroundSize = "50px 50px"
              previewContainer.style.backgroundRepeat = "repeat"
              break
            case "Graphic Print":
              // Simplified representation
              previewContainer.style.backgroundImage = 'url("/placeholder.svg?height=100&width=100")'
