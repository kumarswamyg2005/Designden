  ...
  res.on('end', () => {
    try {
        ...
        for (const [method, operation] of Object.entries(pathItem)) {
          if (['get', 'post', 'put', 'delete', 'patch'].includes(method)) {
            ...
            if (!operation.responses || Object.keys(operation.responses).length === 0) {
              opsWithoutResponses++;
              errorList.push(`❌ No responses: ${method.toUpperCase()} ${path}`);
            }
          }
        }
      }
      ...
      if (opsWithoutResponses > 0) {
        ...
        errorList.slice(0, 10).forEach(e => console.log(`   ${e}`));
      } else {
        console.log('✅ All operations have response definitions');
      }
      ...
      console.log('\n============================================================');
      console.log('✅ CHECK 1 OF 3 COMPLETE\n');
